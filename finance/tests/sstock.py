...
class AppDemo(QWidget):
    def __init__(self):
        ...
        #self.statusBar() 
        #self.initUI()
        #uic.loadUi("C:/DataDisk/Python/finance/tests/sstock.ui", self)
        uic.loadUi("C:/DataDisk/Python/finance/tests/sstock.ui", self)
        ...
        #self.lblProgress.setText("데이터를 모으기 시작하였습니다.")
        self.btnWholeDataCollect.clicked.connect(self.Whole_Data_Collect)
        #self.statusBar.showMessage('In progress') #Added
        #self.lblProgress.setText("모든 데이터를 모았습니다.")

        #self.btnPlotETF.clicked.connect(self.Plot_Chart)
        self.btnSelectBestStock.clicked.connect(self.Select_Best_Stock)
        self.btnSelectedStockPrice.clicked.connect(self.Selected_Stock_Price)
        
        self.cb_multi_plot=QComboBox(self)
        self.cb_multi_plot.setGeometry(500, 500, 250, 40)
        self.cb_multi_plot.setFont(QFont('HY동녘B', 12))

        cb_multi_plot_list = ["보유", "1오일거래량세배", "2볼린저돌파", "ETF", "관심종목"]
        self.cb_multi_plot.addItems(cb_multi_plot_list)
        self.cb_multi_plot.activated[str].connect(self.show_multi_plot) 

        self.qlabel = QLabel(self)
        self.qlabel.setGeometry(500, 460, 500, 40)
        self.qlabel.setFont(QFont('HY동녘B', 12))
        #self.qlabel.move(400,450)

        self.setGeometry(50,50,320,200)
        self.setWindowTitle("QLineEdit Example")
        self.show()

        #self.btnJumpStay.clicked.connect(self.resultupdate)    # 거래량 3배 이후 조정 또는 볼린저 볼파 후 조정
        #self.btnResultUpdate.clicked.connect(self.resultupdate)    #선정된 ETF
        #self.btnSendEMail.clicked.connect(self.sendmail)

        QToolTip.setFont(QFont("SansSerif", 10))
        
        self.btnWholeDataCollect.setToolTip('현재의 주가와 기술적인 지표를 계산하여 DB에 저장합니다.')
        self.btnPlotETF.setToolTip('주가 그래프를 보여줍니다.')

        #self.btnSearchNews.setToolTip('뉴스를 검색하고 Mysql DB wnews "wwindow" 테이블에 저장합니다.')
        #self.btnResultReview.setToolTip('DB에 저장된 검색결과를 <b>"NewsCollector.xlsx"</b>에 기록합니다. 파일을 열고 필요한 뉴스인지 확인해 주세요')
        ...
        self.show()
        #self.lblProgress.setText("종료하였습니다")
    ######################################################################################
    ...
    def show_multi_plot(self, text):
        ...
        if text=="1오일거래량세배":
            ...
            sql+=" and (선정이유='1오일거래량세배' ) order by 선정이유"# limit 8"

            total_selected_stock = pd.read_sql_query(sql, connPy)
            self.qlabel.setText(text+" plot 을 준비하고 있습니다.")
        elif text=="2볼린저돌파":
            sql=f"SELECT DISTINCT SaveDate, 종목명, 종목코드, 선정이유 FROM selected_stock WHERE SaveDate={p_day[0][0]}"
            sql+=" and (선정이유='2볼린저돌파') order by 선정이유"

            total_selected_stock = pd.read_sql_query(sql, connPy)
            self.qlabel.setText(text+" plot 을 준비하고 있습니다.")

        elif text=="보유":
            total_selected_stock = pd.read_excel('C:/DataDisk/Python/finance/tests/보유.xlsx', sheet_name='보유')
            total_selected_stock =total_selected_stock[['종목명','선정이유']]
            ...
            total_selected_stock = pd.read_excel('C:/DataDisk/Python/finance/tests/보유.xlsx', sheet_name='ETF')
            total_selected_stock =total_selected_stock[['종목명','선정이유']]
            sql="SELECT 종목코드, 종목명 FROM stock_code_list"
            stock_code = pd.read_sql_query(sql, connPy)
            total_selected_stock = pd.merge(total_selected_stock, stock_code)
            ...
            total_selected_stock = pd.read_excel('C:/DataDisk/Python/finance/tests/보유.xlsx', sheet_name='관심종목')
            total_selected_stock =total_selected_stock[['종목명','선정이유']]
            sql="SELECT 종목코드, 종목명 FROM stock_code_list"
            stock_code = pd.read_sql_query(sql, connPy)
            total_selected_stock = pd.merge(total_selected_stock, stock_code)
            ...
            for row in range(1,rowC+1):
                for col in range(1,colC+1):
                    if len(selected_stock)>((row-1)*2+col-1):
                        df=[]
                        df=self.make_df(df,str(selected_stock.종목코드[(row-1)*2+col-1])) # ]selected_stock[(row-1)*2+col-1][2]) 
    ...
    def add_trace(self,fig,df,row,col):
        ...
        fig.add_trace(go.Scatter(
            ...
            ,row=row,col=col)
        fig.add_trace(go.Scatter(
            x=df.index,y=df.resultSUMMA10,line=dict(color='rgb(153, 0, 53)',width=2),name='resultSUMMA10')
            ,row=row,col=col)
        fig.add_hline(
        ...
        fig.add_trace(go.Scatter(
            ...
            ,row=row,col=col)              
        fig.add_trace(go.Scatter(
            x=df.index,y=df.pVMA20,line=dict(color='rgb(0, 0, 204)',width=2),name='VMA20',    
            xperiodalignment="middle",hovertemplate="%{y}%{_xother}"),row=row,col=col,)
        fig.add_trace(go.Scatter(
            x=df.index,y=df['pMA5-pMA20'],line=dict(color='rgb(0, 204, 0)',width=2),name='pMA5pMA20'),row=row,col=col)              
        fig.add_hline(
    ...
    def make_plot(self,fig,colC,rowC):
        ...
        if colC==1:
            fig['layout'].update(height=rowC*300, width=2000) #,title={'text': '<b>'+df.종목명[0]+'</b>', 'font': {'size': 20}})
        else:
            fig['layout'].update(height=rowC*300, width=1000) #,title={'text': '<b>'+df.종목명[0]+'</b>', 'font': {'size': 20}})
    ...
    def Selected_Stock_Price(self):
        ...
        self.lblProgress.setText("최고의 종목에 대한 이후 주가변화를 모두 저장하였습니다.")
    ...
    def Select_Best_Stock(self):
        ...
        self.lblProgress.setText("가장 좋은 종목을 선정완료하였습니다.")
    ...
    def stock_market_open_date(self):   #주식장이 열리는 날이면 tradingdate 에 날자를 기록
        ...
        try:
            ...
            #self.lblProgress.setText("네이버 주식에서 개장일을 가져오는데 실패하였습니다.")
    ...
    def Data_Collector(self, dDate, basic_info_mission):
        ...
        if not basic_info_mission == 'Collected':
            ...
            try:
                ...
                #self.lblProgress.setText(dDate, '기본정보 저장실패')
                pass
            ...
            try:    # ETF
                ...
                #self.lblProgress.setText("ETF", dDate, '2/2 ETF 저장성공','하루의 결과를 완료', datetime.datetime.now().strftime("%H:%M:%S"))
            except:
                print('ETF 저장실패', dDate)
                #self.lblProgress.setText('ETF 저장실패', dDate)
                pass
            print("Basic_Info", dDate, '하루의 결과를 완료', datetime.datetime.now().strftime("%H:%M:%S"))
            #self.lblProgress.setText("Basic_Info", dDate, '하루의 결과를 완료', datetime.datetime.now().strftime("%H:%M:%S"))
        ...
        ##########################################################################################################################
                ...
                try:      
                    ...
                    if dfshort.empty==False:                         
                        ...
                        #self.lblProgress.setText(shorting_update_stock_code.loc[idx, "종목명"], '에 공매도 정보가 없습니다. 상장 전이죠?')
                        pass
                    ...
                    try:
                        if dfshort.empty==False:    #공매도 데이터가 있는 경우만 날자를 기록
                            ...
                            #self.lblProgress.setText(str(str(idx+1)+"/"+str(len(shorting_update_stock_code))), dfshort.index[kk].strftime("%Y%m%d"), shorting_update_stock_code.loc[idx, '종목명'],'공매도 저장성공')
                    ...
                    #self.lblProgress.setText(shorting_update_stock_code.loc[idx, "종목명"],'공매도 저장실패')
    ...
    def Ichimoku_Cloud(self, df): #일목균형표 계산
        global d
        d = df.sort_index(ascending=True)#False) # my Live NSE India data is in Recent -> Oldest order
        ...
        d['전환선'] = tenkan_sen
        d['기준선'] = kijun_sen
        d['선행스팬1'] = senkou_span_a
        d['선행스팬2'] = senkou_span_b
        d['후행스팬'] = chikou_span    

        # 현재의 후행스팬은 존재하지 않는다.
        return d.sort_index(ascending=True)

    def HA(self, df): # Heikin Ash 계산
    ...
    def TA_Calc(self): # TA-LIB 계산
        ...
        self.lblProgress.setText("TA 를 계산하기 위하여 데이터를 가져오고 있어요 시간이 필요합니다.")
        ...
        self.lblProgress.setText("TA 를 계산하기 위하여 데이터를 가져왔습니다. 오래 기다렸죠. TA를 계산합니다.")
        try:
            for jj in range(len(df_stock_code)):
                #print(str(df_stock_code.loc[jj, '종목코드']) +'종목에 대하여 TA 계산을 시작합니다.')
                #self.lblProgress.setText(str(df_stock_code.loc[jj, '종목코드']) +'종목에 대하여 TA 계산을 시작합니다.')
                start = time.time()
                df=[]
                #df[df['country'] == '한국']
                ...
                if df.empty==False:   # 정보가 daily 에 있다면
                    ...
                    #macd=macd.replace(np.nan,0.1)
                    #macdSignal=macdSignal.replace(np.nan,0.1)
                    #macdHist=macdHist.replace(np.nan,0.1)
                    ...
                    #df  = df.fillna(0)
                    #########################################################################
                    self.HA(df) # Heikin Ash 
                    self.Ichimoku_Cloud(df)  #일목균형표
                    #일목균형표에서 계산된 data frame d 를 결합
                    #df = pd.merge(df, d, left_on='거래날자', right_on='거래날자', how='outer')
                    df=pd.merge(df, d)
                    # >= 이 올바른 표현
                    ...
                    for idx in range(len(df)):
                        ...
                        sql +=" VALUES"
                        sql +=" (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
                    ...
                    #self.lblProgress.setText(jj+'/'+len(df_stock_code)," 저장했어요, 저장시간", time.time()-start)
                else:
                    print("종목코드 "+df_stock_code.loc[jj, '종목코드']+" 정보가 없어 보조지표를 계산할 수 없습니다. 왜 daily 에 정보가 없는지 확인해야 합니다. 추후 확인필요")
                    #self.lblProgress.setText("종목코드 "+df_stock_code.loc[jj, '종목코드']+" 정보가 없어 보조지표를 계산할 수 없습니다.")
                    pass
        except:
            pass
