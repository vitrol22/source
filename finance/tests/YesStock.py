...
class MainWindow(QMainWindow):
    ...
    def createGridLayout(self):
       ...
       #ui->labelName->setStyleSheet("color: #FF0000");
        ...
        cb_list_best_stock = [
            "014 Stochastic retesting",
            "015 MACD retesting",
            "026 5~20일 전 선택된 후 반등",
            "001 오일거래 세배 눌림",
            "002 오일거래 세배 최초",
            "003 볼린저 상단 눌림",
            "004 볼린저 상단 최초",
            ...
            if plot_stock.empty == False:
                for ii in range(len(plot_stock)):
                    ...
                    fig.add_trace(go.Scatter(
                        x=df.index,y=df.pup_band,opacity=0.9,line=dict(color='rgb(204, 0, 0)',width=2),name='up_band'),
                        secondary_y=False)
                    ...
                    fig.add_trace(go.Scatter(
                        x=df.index,y=df.plow_band,mode="lines",fill="tonexty",fillcolor="rgba(0, 0, 200, 0.1)",line=dict(color="rgba(0, 0, 255, 0.2)",width=4),))
                    ...
                    fig.add_trace(go.Scatter(
                        x=df.index,y=df.indicator_point,line=dict(color='rgb(153, 0, 53)',width=4),name='indicator_point'))
                    fig.add_trace(go.Scatter(
                        x=df.index,y=df.indicator_point_MA5,line=dict(color='rgb(153, 0, 53)',width=10),name='indicator_point_MA5'))

                    fig.add_hline(
                        y=40,
                        line_dash='longdash',
                        line=dict(color='rgb(153, 0, 53)',width=4),
                    ...
                    fig.add_trace(go.Scatter(
                        x=df.index,y=df.pMA5,line=dict(color='rgb(0, 204, 204)',width=4),name='MA5') ) 
                    fig.add_trace(go.Scatter(
                        x=df.index,y=df.pMA20,line=dict(color='orange',width=4),name='MA20'))              
                    fig.add_trace(go.Scatter(
                        x=df.index,y=df.pMA60,line=dict(color='green',width=4),name='MA60'))
                    fig.add_trace(go.Scatter(
                        x=df.index,y=df.pMA120,line=dict(color='black',width=4),name='MA120'))
                    #fig.add_trace(go.Scatter(x=df.index,y=df.ppsar,line=dict(color='black',width=3),name='ppsar'))

                    fig.add_trace(go.Scatter(
                        x=df.index,y=df.pVMA5,line=dict(color='rgb(0, 0, 204)',width=4),name='VMA5'))              
                    fig.add_trace(go.Scatter(
                        x=df.index,y=df.pVMA20,line=dict(color='rgb(0, 0, 204)',width=8),name='VMA20',    
                    ...
                    fig.add_trace(go.Scatter(
                        x=df.index,y=df['pMA20-pMA60'],line=dict(color='rgb(0, 204, 0)',width=8),name='pMA20pMA60'))                                     
                    fig.add_hline(
                        y=50,
                        line_dash='longdash',
                    ...
                    fig.add_hline(
                        ...
                        annotation_text='Indicator Point Limit',
                        annotation_position="bottom right",
                        annotation_font_size=40,
                        annotation_font_color='rgb(200, 0, 0)')

                    #fig.data[1].update(xaxis='x2')
                    fig.update_layout(width=2400*2, height=1000*2)
                    fig.update_layout(xaxis_rangeslider_visible=False)

                    fig.update_layout(
                        title=df.종목명[0],
                        #font=dict(size=40,color='#60606e',family='Franklin Gothic'),
                        showlegend=False,
                        margin=dict(t=150, b=70, l=100, r=100),

                        xaxis_rangeslider_visible=False,            
                        xaxis_title=None, yaxis_title=None,
                        #font=dict(color='#8a8d93'),
                        font=dict(color='#8a8d93', size = 40),
                        hoverlabel=dict(bgcolor="white",font_size=40,font_family="Rockwell"),
                        legend=dict(orientation="h", yanchor="bottom", y=1, xanchor="center", x=0.5))


                    fig.update_xaxes(showspikes=True,spikecolor="green",spikethickness=5,spikedash="dot",spikesnap="cursor",spikemode="across", gridwidth=5)
                    fig.update_yaxes(showspikes=True,spikecolor="orange",spikethickness=5,spikedash="dot",spikesnap="cursor",
                        spikemode="across",ticklabelposition="inside top", title=None, gridwidth=5)
                    
                    fig.for_each_annotation(lambda a: a.update(text = a.text + ': 40'))
                    # fig['layout'].update(height=rowC*210*2*2.5, width=2300*2, 
                    #     title={'text': '<b>'+str(selected_stock.선정이유[idx_browser_num])+'</b>', 'font': {'size': 50},'x':0.5,'xanchor': 'center'})
    ...
    def def_cb_compare_stock(self, text):
        ...
        #fig.update_layout(hoverlabel=dict(bgcolor="white",font_size=40,font_family="Rockwell"))
        #fig.show()
        self.browser.setHtml(fig.to_html(include_plotlyjs='cdn'))
        ...
        fig = go.Figure(go.Scatter(
            #x=df_plot_data.index,y=df_plot_data.해운,opacity=0.9,line=dict(color='rgb(204, 0, 0)',width=4)))
    ...
    def show_multi_plot(self, text):
        ...
        if text=="001 오일거래 세배 눌림" \
            ...
            or text=="014 Stochastic retesting" \
            or text=="015 MACD retesting" \
            or text=="016 3일간 눌림목" \
            ...
            or text=="026 5~20일 전 선택된 후 반등" \
            or text=="030 3번이상 선정":
            #or text=="027 5~20일 전 선택된 후 오늘 재반등<50" \            
        ...
        if total_selected_stock.empty==False:
            ...
            for idx_browser_num in range(browser_num):
                ...
                fig = make_subplots(
                    ...
                    subplot_titles =[str(selected_stock.종목명[idx])+', '+str(selected_stock.SaveDate[idx]) for idx in np.arange(len(selected_stock))]
                    ) #그래프 간 간격        
                df=[]
                ...
                fig.update_layout(
                    margin=dict(l=50, r=40, t=200, b=0),
                    showlegend=False,
                    font=dict(color='#8a8d93', size = 16+16),

                    hoverlabel=dict(bgcolor="white", font_size = 40, font_family="Rockwell")
                    )

                fig.update_yaxes(range=[0, 100], showgrid=True,gridwidth=4, gridcolor='white')
                fig.update_xaxes(showgrid=True, gridwidth=4, gridcolor='white', dtick=10)

                #fig['layout'].update(height=rowC*210*2*2.5, width=2300*2, title=str(selected_stock.선정이유[idx_browser_num]))
                fig['layout'].update(height=rowC*220*5, width=2300*2, 
                    title={'text': '<b>'+str(selected_stock.선정이유[idx_browser_num])+'</b>', 
                        'font': {'size': 50},'x':0.5,'y':0.99,'xanchor': 'center', 'yanchor': 'top'})


                # fig['layout'].update(height=rowC*210*2*2.5, width=2300*2, title={
                #     'text': str(selected_stock.선정이유[idx_browser_num]),
                #     'y':1,
                #     'x':0.5,
                #     'xanchor': 'center',
                #     'yanchor': 'top'})
                ...
                fig.update_annotations(font_size=20+30)
    ...
    def make_df(self,df,종목코드):
        ...
        df.to_excel('C:/DataDisk/df.xlsx')
        if df.empty==False:
            # 최대가격 검색
    ...
    def sub_add_trace(self,fig,df,row,col):
        ...
        fig.add_trace(go.Candlestick(
            ...
            ,row=row,col=col)
        fig.add_trace(go.Scatter(
            x=df.index,y=df.indicator_point,line=dict(color='rgb(153, 0, 53)',width=2),name='indicator_point')
            ,row=row,col=col)
        fig.add_trace(go.Scatter(
        ...
        fig.add_hline(
            y=40,
            line_dash='longdash',
            line=dict(color='rgb(153, 0, 53)',width=2),
            annotation_text='총점관심',
            annotation_position="bottom right",
            annotation_font_size=40,
            annotation_font_color='rgb(153, 0, 53)')
        ...
        '''
        fig.add_trace(go.Scatter(
            x=df.index,y=df.pMA20,line=dict(color='orange',width=2),name='MA20')
            ,row=row,col=col)              
        fig.add_trace(go.Scatter(
            x=df.index,y=df.pMA60,line=dict(color='green',width=2),name='MA60')
            ,row=row,col=col)
        fig.add_trace(go.Scatter(
            x=df.index,y=df.pMA120,line=dict(color='black',width=2),name='MA120')
            ,row=row,col=col)
        fig.add_trace(go.Scatter(
            x=df.index,y=df['pMA5-pMA20'],line=dict(color='rgb(0, 200, 0)',width=2),name='pMA5pMA20')
            ,row=row,col=col)   
        fig.add_trace(go.Scatter(
        ...
        fig.add_hline(
            y=70,
            line_dash='longdash',
            line=dict(color='rgb(200, 0, 0)',width=2),
            annotation_text='Indicator Point Limit',
            annotation_position="bottom right",
            annotation_font_size=40,
            annotation_font_color='rgb(200, 0, 0)')
    ...
    def make_plot(self,fig,colC,rowC):
        ...
        fig.update_xaxes(showspikes=True,spikecolor="green",spikethickness=5,spikedash="dot",spikesnap="cursor",spikemode="across", gridwidth=2)
        fig.update_yaxes(showspikes=True,spikecolor="orange",spikethickness=5,spikedash="dot",spikesnap="cursor",
            spikemode="across",ticklabelposition="inside top", title=None, gridwidth=2)
        ...
        try:
            for jj in range(len(df_stock_code)):
                ...
                if df.empty==False:   # 정보가 daily 에 있다면
                    ...
                    #df['slowk'], df['slowd'] = ta.STOCH(df['High'], df['Low'], df['Close'], fastk_period=12, slowk_period=5, slowk_matype=0, slowd_period=5, slowd_matype=0)
                    ...
                    df.loc[df['Close'] > df['MA5'], 'indicator_point'] = df['indicator_point'] + 5              #1
                    df.loc[df['MA5'] > df['MA20'], 'indicator_point'] = df['indicator_point'] + 5               #2
                    df.loc[df['MA20'] > df['MA60'], 'indicator_point'] = df['indicator_point'] + 5              #3
                    df.loc[df['Close'] > df['psar'], 'indicator_point'] = df['indicator_point'] + 5             #4
                    df.loc[df['macd'] > df['macdSignal'], 'indicator_point'] = df['indicator_point'] + 5        #5
                    df.loc[df['slowk'] > df['slowd'], 'indicator_point'] = df['indicator_point'] + 5            #6
                    df.loc[df['RSI'] > df['RSIMA5'], 'indicator_point'] = df['indicator_point'] + 5             #7
                    df.loc[df['pdi'] > df['mdi'], 'indicator_point'] = df['indicator_point'] + 5                #8
                    df.loc[df['HA_Close'] > df['HA_Open'], 'indicator_point'] = df['indicator_point'] + 5       #9
                    df.loc[df['Close'] > df['mid_band'], 'indicator_point'] = df['indicator_point'] + 5         #10
                    df.loc[df['Volume'] > df['VMA5'], 'indicator_point'] = df['indicator_point'] + 5            #11
                    df.loc[df['VMA5'] > df['VMA10'], 'indicator_point'] = df['indicator_point'] + 5             #12
                    df.loc[df['VMA10'] > df['VMA20'], 'indicator_point'] = df['indicator_point'] + 5            #13
                    df.loc[df['willr120'] > df['AVGwillr120'], 'indicator_point'] = df['indicator_point'] + 5   #14
                    df.loc[df['willr14'] > df['AVGwillr14'], 'indicator_point'] = df['indicator_point'] + 5     #15
    ...
    def best_stock_to_db(self, calc_date):
        ...
        for idx in range(len(dfSelected)):
            sql="INSERT IGNORE INTO selected_stock (SaveDate, 종목명, 종목코드, 선정이유) VALUES ("
            sql+="'"+dfSelected.iloc[idx,0]+"','"+dfSelected.iloc[idx,1]+"','"+dfSelected.iloc[idx,2]+"','014 Stochastic retesting')"
            cur.execute(sql)
            connPy.commit()
        ...
        for idx in range(len(dfSelected)):
            sql="INSERT IGNORE INTO selected_stock (SaveDate, 종목명, 종목코드, 선정이유) VALUES ("
            sql+="'"+dfSelected.iloc[idx,0]+"','"+dfSelected.iloc[idx,1]+"','"+dfSelected.iloc[idx,2]+"','015 MACD retesting')"
            cur.execute(sql)
            connPy.commit()
        ...
        sql+=" AND ss.SaveDate<'" + dfDate.거래날자[5] + "'"
        sql+=" AND ta.종목코드 = tb.종목코드"
        sql+=" AND ta.종목코드 = tc.종목코드"
        sql+=" AND ta.종목코드= ss.종목코드 "
        #sql+=" AND ta.indicator_point_MA5>50"
        sql+=" AND (ta.MA5-ta.MA20)>(ta.MA20-ta.MA60)"      #(오늘 5이평-20이평)>(20 이평-60이평)
        sql+=" AND (tc.indicator_point_MA5-tb.indicator_point_MA5)>0"  #어제 오이평 하락
        sql+=" AND (tb.indicator_point_MA5-ta.indicator_point_MA5)<0"  #오늘 5이평 상승
        sql+=" order by ta.indicator_point_MA5 DESC;"

        dfSelected = pd.read_sql_query(sql, connPy)
        for idx in range(len(dfSelected)):
            sql="INSERT IGNORE INTO selected_stock (SaveDate, 종목명, 종목코드, 선정이유) VALUES ("
            sql+="'"+dfSelected.iloc[idx,0]+"','"+dfSelected.iloc[idx,1]+"','"+dfSelected.iloc[idx,2]+"','026 5~20일 전 선택된 후 반등')"
            cur.execute(sql)
            connPy.commit()
        print(dfDate.거래날자[0], '선정 8~20 재반등 종목에 대한', '계산을 완료하였습니다.')

        # #선정 8~20 오늘 재반등
        # # 8~20 일 전에 재반등한 종목에 대한 경향 분석, 공부하기 위한 
        # sql="SELECT distinct ta.거래날자, ta.종목명, ta.종목코드 "
        # sql+=" FROM ta ta, ta tb, ta tc, selected_stock ss"
        # sql+=" WHERE ta.거래날자 = '" + dfDate.거래날자[0] + "'"
        # sql+=" AND tb.거래날자 = '" + dfDate.거래날자[1] + "'"
        ...
        # for idx in range(len(dfSelected)):
        #     sql="INSERT IGNORE INTO selected_stock (SaveDate, 종목명, 종목코드, 선정이유) VALUES ("
        #     sql+="'"+dfSelected.iloc[idx,0]+"','"+dfSelected.iloc[idx,1]+"','"+dfSelected.iloc[idx,2]+"','027 5~20일 전 선택된 후 오늘 재반등<50')"
        #     cur.execute(sql)
        #     connPy.commit()
        # print(dfDate.거래날자[0], '선정 8~20 재반등 종목에 대한', '계산을 완료하였습니다.')
        ...
        for ii in range(len(df_stock_code)):
            ...
            try:
                ...
                for idx in range(len(df_annualT)):
                    ...
                    try:
                        delete_sql=f"DELETE FROM naver_finance WHERE 종목코드='{df_stock_code.iat[ii,1]}' and 시기='{df_annualT.iat[idx,0]}' and 년분기='년간'"
                ...
                for idx in range(len(df_quaterT)):
                    ...
                    try:
                        delete_sql=f"DELETE FROM naver_finance WHERE 종목코드='{df_stock_code.iat[ii,1]}' and 시기='{df_quaterT.iat[idx,0]}' and 년분기='분기'"
                        cur.execute(delete_sql)
                        connPy.commit()
...
# 계산을 다시하기 위하여 데이터를 지운다
# update stock_code_list set ta_calc_date='20190101';
# delete from ta 

# ### selected_stock 다시 계산
# delete from selected_stock
